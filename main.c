...
uint16_t eeepromaddress = 0;
uint16_t eeepromaddressMax = 0;
...
int main(void)
{
  ...
  for (size_t i = 0; i < sizeof(patterns) / sizeof(patterns[0]); i++) {
          write_to_address(eeepromaddress, patterns[i]);
          eeepromaddress++;
      }
  eeepromaddressMax = eeepromaddress;
  eeepromaddress = 0;
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET) {
	      // Button is pressed, change the timer period to 0.5 seconds
	      htim16.Instance->ARR = 500;
	  } else {
	      // Button is not pressed, revert to the original 1-second period
...
void TIM16_IRQHandler(void)
{
	...
	binary_value = read_from_address(eeepromaddress);
	if(eeepromaddress < eeepromaddressMax){
		eeepromaddress++;
	}else{
		eeepromaddress = 0;
	}
	...
	for (size_t i = 0; i < sizeof(patterns) / sizeof(patterns[0]); i++) {
		found_in_array = patterns[i] == binary_value;
		if(found_in_array) break;
	      }
	//If found in array then write to LEDS
	if(found_in_array == false){
		binary_value = 0b00000001;
	}
	//Set LEDS
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, (binary_value & (1 << 0)) ? GPIO_PIN_SET : GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, (binary_value & (1 << 1)) ? GPIO_PIN_SET : GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_2, (binary_value & (1 << 2)) ? GPIO_PIN_SET : GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, (binary_value & (1 << 3)) ? GPIO_PIN_SET : GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, (binary_value & (1 << 4)) ? GPIO_PIN_SET : GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, (binary_value & (1 << 5)) ? GPIO_PIN_SET : GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_6, (binary_value & (1 << 6)) ? GPIO_PIN_SET : GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_7, (binary_value & (1 << 7)) ? GPIO_PIN_SET : GPIO_PIN_RESET);
